	...
	public class DocPmP : System.Web.UI.Page
	{
		...
		private void SalvaDifferibili()
		{
			...
			if (UploadFile.PostedFile!=null && UploadFile.PostedFile.FileName!="") 
			{
				...
				if (IdFile<1)
					...
					lblMessage.Text="Il Documento è stato inserito con successo.";
			}
		}
		#endregion
			...
			if(arrstr.Length< 2 || arrstr2.Length < 2)
				{
					...
					if(DrTipoDocumenti.SelectedValue=="1")
						{
							lblMessage.Text="Il File selezionato non è un file Valido per l'invio, il formato deve essere:PAM_ANNO_NOMEEDIFICIO(PROVINCIA).xls!";
							return true;
						}
		...
		private bool IsValidDocAnnoH3G()
		{
			...
			if(arrstr.Length< 2 || arrstr2.Length < 2)
			{
				...
				if(DrTipoDocumenti.SelectedValue=="1")
				{																										
					lblMessage.Text="Il File selezionato non è un file Valido per l'invio, il formato deve essere:NOMEEDIFICIO_PamAnno_Versione.xls!";
					return true;
				}
				else if(DrTipoDocumenti.SelectedValue=="2")
				{
		...
		private void SendDoc()
		{
			...
			{
				SalvaDifferibili();
				//Invio della Mail in modalità asincrona di tracciatura
		...
		private void SendMail(string FileName)
		{
			...
			else
			{
				...
				//FileExcel=CreaFileXLS(Out,CodiceEdificio,Mese,Anno);
				//Il file da passare non è + FileName ma FileExcel
                FileExcel=FileName;
		...
		private void Send()
		{
			...
			//SendTracker();
			//Non ivio le email se il flag non è spuntato
