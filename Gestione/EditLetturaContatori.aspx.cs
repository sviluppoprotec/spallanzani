...
namespace TheSite.Gestione
{
	...
	public class EditLetturaContatori : System.Web.UI.Page
	{
		...
		protected S_Controls.S_TextBox TXTUMISURA;
		protected System.Web.UI.WebControls.TextBox txtsorain;
		protected System.Web.UI.WebControls.RequiredFieldValidator RvfTXTUMISURA;
...
//			txtsorainmin.Attributes.Add("onblur","formatNum(this.id);");
			...
			s_funz= "Formatta1('" + txtsorainmin.ClientID + "');";
			TxtValoreLetturaDec.Attributes.Add("onblur",s_funz);


			TxtValoreLetturaInt.Attributes.Add("onkeypress","SoloNumeri();");
			TxtValoreLetturaInt.Attributes.Add("onpaste","return false;");

//			this.BtnSalva.Attributes.Add("onclick","ControllaOra();");
...
//							txtsorainmin.Text= Convert.ToString(appo1).PadLeft(2,'0');
						...
						if (_Dr["oralettura"] != DBNull.Value)
						{
						txtsorain.Text=_Dr["oralettura"].ToString();
						}
						if (_Dr["minlettura"] != DBNull.Value)
						{
							txtsorainmin.Text=_Dr["minlettura"].ToString();
						}
						
						if (_Dr["energia"] != DBNull.Value)
							CmbsEnergia.SelectedValue= (string) _Dr["energia"];	
					
						if (_Dr["umisura"] != DBNull.Value)
							TXTUMISURA.Text= (string) _Dr["umisura"];	
						...
						BindApparecchiaturaSV(_Dr["Servizio"].ToString());	
		...
		private void BindApparecchiatura(string Servizio)
		{
			
			this.cmbsApparecchiatura.Items.Clear();
		
			Classi.ClassiAnagrafiche.LetturaContatori  _LCCONT = new TheSite.Classi.ClassiAnagrafiche.LetturaContatori(Context.User.Identity.Name);
			...
			if (RicercaModulo1.TxtCodice.Text!="")
			{
				...
				s_Servizio.Index = 1;
				s_Servizio.Value =Int32.Parse(Servizio);
				_SColl.Add(s_Servizio);

				_MyDs = _LCCONT.GETDATASTDCONTATORI(_SColl).Copy();
		...
		private void BindApparecchiaturaSV(string Servizio)
		{
		...
		private void Salva(string operazione)
		{
			...
			s_p_datalettura.Size=15;
			s_p_datalettura.Value =CalendarPicker1.Datazione.Text;
			_SCollection.Add(s_p_datalettura);
			
			//Ora Lettura
			S_Controls.Collections.S_Object s_p_oralettura = new S_Object();
			s_p_oralettura.ParameterName = "p_oralettura";
			s_p_oralettura.DbType = CustomDBType.VarChar;
			s_p_oralettura.Direction = ParameterDirection.Input;
			s_p_oralettura.Index = 7 ;
			s_p_oralettura.Size=2;			
			s_p_oralettura.Value = txtsorain.Text;
			_SCollection.Add(s_p_oralettura);
			

			//Ora Lettura
			S_Controls.Collections.S_Object s_p_minlettura = new S_Object();
			s_p_minlettura.ParameterName = "p_minlettura";
			s_p_minlettura.DbType = CustomDBType.VarChar;
			s_p_minlettura.Direction = ParameterDirection.Input;
			s_p_minlettura.Index = 8 ;
			s_p_minlettura.Size=2;			
			s_p_minlettura.Value = txtsorainmin.Text;
			_SCollection.Add(s_p_minlettura);



			//Unità di Misura
			S_Controls.Collections.S_Object s_p_umisura = new S_Object();
			s_p_umisura.ParameterName = "p_umisura";
			s_p_umisura.DbType = CustomDBType.VarChar;
			s_p_umisura.Direction = ParameterDirection.Input;
			s_p_umisura.Index = 9 ;
			s_p_umisura.Size=15;
			s_p_umisura.Value =TXTUMISURA.Text ;
			_SCollection.Add(s_p_umisura);	
			try
			{
				if (operazione == "I")
				{			
