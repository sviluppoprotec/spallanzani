		...
		private void execute()
		{
			...
			if (this.S_LblCodiceDoc.Text=="")
			{
				...
				if(CheckRinomina.Checked==true)
				{
					...
					{
						S_Lblerror.Text="Il file è già presente impossibile inserire. Rinominare il file o cancellare quello vecchio."; 
						return;
					} 
				}
				else
				{
					...
					{
						S_Lblerror.Text="Il file è già presente impossibile inserire. Rinominare il file."; 
						return;
					}
				}
			}
			else//si tratta di una modifica
			{
				...
				//altrimenti mantengo la stessa immagine  ma recupero l'estensione
				if(IsSelectFile()==true)
				{
					...
					if (CheckRinomina.Checked==false && S_LblFileName.Text!=System.IO.Path.GetFileName(Uploader.PostedFile.FileName))
					{
						...
						//imposto a true per l'eliminazione del file
			...
			s_p_nomedwf.Index =_SColl.Count;
			s_p_nomedwf.Size =50;
			...
			s_p_nfascicolo.Index =_SColl.Count;
			...
			s_p_datarilascio_cpi.Index =_SColl.Count;
			...
			s_p_datascadenza_cpi.Index =_SColl.Count;
			...
			s_p_dataparere_cpi.Index =_SColl.Count;
			...
			s_p_collaudo.Index =_SColl.Count;
			...
			s_p_matricola.Index =_SColl.Count;
			...
			s_p_dataprimaverifica.Index =_SColl.Count;
			...
			s_p_descrizione.Index =_SColl.Count;
			...
			s_p_documento_id.Index =_SColl.Count;
			...
			s_p_piano_id.Index =_SColl.Count;
			...
			s_p_iddocdwf_tipologie.Index =_SColl.Count;
			...
			s_p_iddocdwf_categorie.Index =_SColl.Count;
			...
			s_p_paginedocumento.Index =_SColl.Count;
			...
			s_p_codicedwf.Index =_SColl.Count;
			...
			s_p_dimensionefile.Index =_SColl.Count;
		...
		private string PostFile(string RenameFile)
		{
			...
			if (Uploader.PostedFile!=null && Uploader.PostedFile.FileName!="") 
			{
				try
				{
					...
					// PRIMO IF IL FILE VA RINOMINATO? SI-NO
					//caso NO
//					if (RenameFile=="")
//						// categoria file POI
//									if (int.Parse(cmbsCategoria.SelectedValue.Split(Convert.ToChar(","))[0])==14)
//									{
//										for (int i = 1; i <= 18; i++)
//										{
//										if (i<10)
//										{fileName="0"+i.ToString()+ "_" + fileNamerin;
//											destPath  = System.IO.Path.Combine(folderChild, fileName);
//											Uploader.PostedFile.SaveAs(destPath);}
//										else 
//										{fileName=i.ToString()+ "_" + fileNamerin;
//											destPath  = System.IO.Path.Combine(folderChild, fileName);
//											Uploader.PostedFile.SaveAs(destPath);}
//										}
//										return fileNamerin;
//									}
//						//tutti i file di categoria diversa da POI
//									else
//									{
//									destPath  = System.IO.Path.Combine(folderChild, fileName);
//									Uploader.PostedFile.SaveAs(destPath);
...
//				}
					if (RenameFile=="")
					{
						fileNamerin=fileNamerin;
					}
					else
					{
						fileNamerin=RenameFile;
						...
						if (int.Parse(cmbsCategoria.SelectedValue.Split(Convert.ToChar(","))[0])==14)
						{
							TheSite.Classi.AnagrafeImpianti.AnagrafeDocDWF  AnagrafeDocDWF_ = new TheSite.Classi.AnagrafeImpianti.AnagrafeDocDWF(Context.User.Identity.Name);
							DataSet _Ds= AnagrafeDocDWF_.GetCodeAllBuildings();
							DataTable dt=_Ds.Tables[0];

//							DataRow dt= _Ds.Tables[0].Rows[0];
							
							foreach (DataRow dr in dt.Rows)
							{
								

								fileName=dr["bl_id"].ToString()+ "_" + fileNamerin;
								destPath  = System.IO.Path.Combine(folderChild, fileName);
								Uploader.PostedFile.SaveAs(destPath);
...
//							}
						...
						{
							destPath  = System.IO.Path.Combine(folderChild, fileNamerin);
							Uploader.PostedFile.SaveAs(destPath);
							return fileName;
						}	
