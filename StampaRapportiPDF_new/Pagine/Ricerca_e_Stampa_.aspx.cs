...
using StampaRapportiPDF_new.WebControls;
using StampaRapportiPDF_new.Classi;
using S_Controls.Collections;
using S_Controls;
using ApplicationDataLayer.DBType;
using TheSite.StampaRapportiPDF_new.Reports;
using System.IO;
using ICSharpCode.SharpZipLib.Zip;
using RptPdf = TheSite.StampaRapportiPDF_new.Schemixsd;
using CrystalDecisions.CrystalReports.Engine;

namespace StampaRapportiPDF_new.Pagine
{
	/// <summary>
	/// Descrizione di riepilogo per Ricerca_e_Stampa.
	/// </summary>
	public class Ricerca_e_Stampa_ : System.Web.UI.Page
	{
		...
		private void Page_Load(object sender, System.EventArgs e)
		{
			...
			PgTlStampaPdf.MainTitle  = "Crea File Report MP";
			...
			if(!IsPostBack)
			{
				riempiCombo("RapportiPdf.bind_edifici_",S_cmbEdificio,"System.String");
				...
				riempiCombo("RapportiPdf.bind_categoria_",S_cmbCategoria,"System.Int32");
		...
		private DataSet BindCmb(string StoredProcedure)
		{
			...
			if(StoredProcedure=="RapportiPdf.bind_edifici_"
				||StoredProcedure=="RapportiPdf.bind_categoria_"
				)
			{
				...
				username.Index = clDatiRicerca.Count;
		...
		private TheSite.StampaRapportiPDF_new.Schemixsd.DatasetReport riempiDatasetRicerca()
		{
			AgganciaDatalayer io_db  = new AgganciaDatalayer();
		...
		{
			Response.Redirect("Ricerca_e_Stampa_.aspx");
		}

		private void S_btnDownLoad_Click(object sender, System.EventArgs e)
		{
			Response.Redirect("Pagina_Download_.aspx");
		}
		...
		private int[] insertNormTblDb(string [] arOdl,int idFile)
		{
			...
			for(int i =0; i<arOdl.Length ; i++)
			{
				...
				pusername.Size = 50;
				pusername.Index = pIndex++;
				pusername.Value = System.Web.HttpContext.Current.User.Identity.Name;
				clDatiUpDb.Add(pusername);


				io_db.NameProcedureDb="RapportiPdf.inserisciTblNorm_";
				int result = io_db.Add(clDatiUpDb);
				arResult[i] = result;
			}
			return arResult;
		...
		private void stampaPdf(TheSite.StampaRapportiPDF_new.Schemixsd.DatasetReport ds,int nOdl,int idFile)
		{
			//MP_Rapporti_C_MP_long_r6 rptLong = new MP_Rapporti_C_MP_long_r6();
			//MP_Rapporti_C_MP_long_r6_2 rptLong = new MP_Rapporti_C_MP_long_r6_2();
			//MP_Rapporti_C_MP_r6 rptShort = new MP_Rapporti_C_MP_r6();

			string pathRptSource = Server.MapPath(Request.ApplicationPath + @"\StampaRapportiPDF_new\Reports\MP_Rapporti_C_MP_long_r6_2.rpt");
		...
		private TheSite.StampaRapportiPDF_new.Schemixsd.DatasetReport riempiDatasetStampa(int idFile)
		{
			AgganciaDatalayer io_db  = new AgganciaDatalayer();
			...
			io_db.NameProcedureDb = "RapportiPdf.get_data_reports_";
			...
			{ 
				...
				DsTipizzato.Tables["MP_REPORT_LONG_NEW"].ImportRow(dsDatiStampa.Tables[0].Rows[i]);
			}
			if(i==0)
			{
