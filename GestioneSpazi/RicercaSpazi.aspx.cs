...
namespace TheSite.GestioneSpazi
{
	...
	public class RicercaSpazi : System.Web.UI.Page
	{
		...
		public string sql_from= String.Empty;
		...
		protected S_Controls.S_Button S_btExcel;
		int startIndex = 0;
		...
		{    
			...
			this.S_btExcel.Click += new System.EventHandler(this.S_btExcel_Click);
		...
		private void BindTuttiPiani()
		{
			...
			DataSet	_MyDs = _Buildings.GetAllPianiRICERCASPAZI();
		...
		private void BindTutteCategorie()
		{
			...
			if (_MyDs.Tables[0].Rows.Count > 0)
			{
				this.cmbsCategoria.DataSource = Classi.GestoreDropDownList.ItemBlankDataSource(
					_MyDs.Tables[0], "CATEGORIA", "ID", "- Selezionare MacroArea -", "");
		...
		private void DtgRicercaSpazi_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if(e.Item.ItemType == ListItemType.Footer)
			{
				...
				if (colonna==2)
				{
						e.Item.Cells[colonna-2].Text ="<b>TOTALE mq</b>" + "<b>&nbsp;&nbsp;&nbsp;"+ totale.ToString("#,##0.00") +"</b>";
					//e.Item.Cells[colonna - 2].HorizontalAlign =HorizontalAlign.Left; 
					e.Item.Cells[colonna-1].Text ="<b>TOTALE mc</b>" + "<b>&nbsp;&nbsp;&nbsp;"+ vtotale.ToString("#,##0.00") +"</b>";
					//e.Item.Cells[0].Text ="<b>"+ totale.ToString("C") +"</b>";
				}
				else
				{
					e.Item.Cells[0].Text= "<b>TOTALE GENERALE</b>";
					e.Item.Cells[colonna - 2].HorizontalAlign =HorizontalAlign.Right; 
					e.Item.Cells[colonna - 1].HorizontalAlign =HorizontalAlign.Right; 
					e.Item.Cells[colonna - 2].Text ="<b>"+ totale.ToString("#,##0.00") +"</b>";
					e.Item.Cells[colonna - 1].Text ="<b>"+ vtotale.ToString("#,##0.00") +"</b>";
				}
		...
		private void CalcolaTotali(DataTable dt)
		{
			foreach(DataRow dr in dt.Rows)
			{
				...
				vtotale+=double.Parse(dr["VALORE_mc"].ToString());
		...
		private void S_btExcel_Click(object sender, System.EventArgs e)
		{
			LoadList();
			//BindingEdifici1(edifici.Value, false);
			BindingEdifici_(edifici.Value, false);
		}
		private void BindingEdifici(string BlId, bool reset)
		{
			...
			if (chkStanze.Checked == true)
			{
				...
				sql_select += "v_rm.RM_ID || '-'  || v_rm.DESCRIZIONE as STANZA ";
				sql_group += " v_rm.RM_ID,v_rm.DESCRIZIONE ";
			}
			...
			if (UserStanze1.DescStanza.ToString() !="")
			{
				if (sql_where != "")
					sql_where += " AND";
				sql_where += " UPPER(v_rm.RM_ID ||'-'|| v_rm.DESCRIZIONE) like '%" + UserStanze1.DescStanza.ToUpper() + "%'" ;
			}
			...
			if (s_txtDestinazione.Text !=String.Empty||chkDestinazione.Checked == true)
			{
				...
				sql_where +=" and v_rm.id_rm_dest_uso=RM_DEST_USO.ID_RM_DEST_USO ";
			...
			if (cmbsConfronto.SelectedValue != String.Empty & s_txtMq.Text!= String.Empty)
			{
				...
				sql_where += " v_rm.AREA " + cmbsConfronto.SelectedValue + " " + s_txtMq.Text.Replace(",",".");
			}
			...
			if (sql_select!="")
				sql_select += ", ";
			sql="SELECT " + sql_select + " SUM(v_rm.AREA) as VALORE_INT, sum(round(v_rm.AREA*cost,2)) as VALORE_mc from v_rm  ";
			...
			sqlCount = "SELECT count(distinct v_rm.RM_ID)  from v_rm ";// JOIN FL ON (FL.Id_Bl = BL.Id) " + 
			//"JOIN RM USING (id_piani, id_bl) JOIN PIANI ON (id_piani = PIANI.id) " +
			//"left JOIN RM_REPARTO ON (RM.ID_RM_REPARTO = RM_REPARTO.ID_RM_REPARTO) " +
			//"left JOIN RM_DEST_USO ON (RM.ID_RM_DEST_USO = RM_DEST_USO.ID_RM_DEST_USO) " +
			//" left JOIN RM_CAT ON (RM.ID_RM_CAT = RM_CAT.ID_RM_CAT) " + sql_where   ;
				...
				)
			{
				...
				cl8.DataField="VALORE_mc";
				cl8.HeaderText="Mc";
				cl8.ItemStyle.HorizontalAlign=HorizontalAlign.Right;
				cl8.DataFormatString="{0:#,##0.00}";
