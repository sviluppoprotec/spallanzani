namespace TheSite.WebControls
{
	...
	public class CostiManodopera : System.Web.UI.UserControl
	{
		...
		private void BindGrid()
		{
			DataTable tbDati;
			ClManCorrettiva ioDati = new ClManCorrettiva();
			DataSet DsManodopera = ioDati.GetListaManodopera(_wrId).Copy() ; // carica elenco manodopera associata alla rdl in griglia
			tbDati = AggiungiColonnaTotProgressivo(DsManodopera.Tables[0]);  // codice superfluo non fanulla

			//per inserire una riga
   ///    inserisce riga vuota aggiunge la riga del totale 
			DataRow o_Dr = tbDati.NewRow();
			o_Dr["ID"]=0;				
			o_Dr["IdAddetto"]=0;
			o_Dr["IdAddettoWR"]=0;
			...
			DataGridEsegui.DataSource = tbDati;
			DataGridEsegui.DataBind();
			lblRecord.Text =  DsManodopera.Tables[0].Rows.Count.ToString();  // totale record inseriti
			///////
		...
		private void DataGridEsegui_ItemDataBound(object sender, System.Web.UI.WebControls.DataGridItemEventArgs e)
		{
			...
			if(e.Item.ItemType== ListItemType.Footer)
			{
				...
				/// qui ci carico cmdAddetto insert !!!!!!!!!!!###########################	Value	"1027;29,03;3° LIVELLO MMC"	string  ###########################
				cmbNomeCognome    = (DropDownList) e.Item.FindControl("cmbAddettoInsert");
			//     NOMECOGNOME                                            ADDETTOINSERT
				...
				// CARICO L'ADDETTO INSERT IN UNA VARIABILE DI TIPO VETTORE ( TRE CAMPI 0=IDADDETTO,1=PREZZOUNITARIO,2=LIVELLO
				if (cmbNomeCognome.SelectedValue=="")
				{
						string nullo =" ; ";
					arValueCmb=nullo.Split(';');
				}
				else
				{arValueCmb =  cmbNomeCognome.SelectedValue.Split(';');}

				// carico la riga del nuovo inserimento come default (il primo campo)

				txtPrezzoUnitario.Attributes.Add("value",arValueCmb[1].ToString());

				//txtLivello.Attributes.Add("value",arValueCmb[0].ToString());   ERRORE
				txtLivello.Attributes.Add("value",arValueCmb[2].ToString());
